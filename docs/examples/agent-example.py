...
if __name__ == "__main__":
    ...
    try:
        next_result = agent.invoke({
            "messages": result['messages'] + [("user", "Please process a refund for that order due to late delivery.")]
            })
    
        final_final_message = next_result['messages'][-1].content
        print(f"Agent response: {final_final_message}\n")
